...
// 키워드 기기별/성별/연령별 API용 스키마 (keyword를 단일 문자열로 받음)
export const KeywordSingleRequestSchema = z.object({
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  timeUnit: z.enum(['date', 'week', 'month']),
  category: z.string(),
  keyword: z.string(), // 단일 문자열
  device: z.enum(['pc', 'mobile', 'all']).optional(),
  gender: z.enum(['m', 'f', 'a']).optional(),
  ages: z.array(z.enum(['10', '20', '30', '40', '50', '60'])).optional()
});

// 카테고리 기기별/성별/연령별 API용 스키마 (category를 단일 문자열로 받음)
export const CategorySingleRequestSchema = z.object({
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  timeUnit: z.enum(['date', 'week', 'month']),
  category: z.string(), // 단일 문자열
  device: z.enum(['pc', 'mobile', 'all']).optional(),
  gender: z.enum(['m', 'f', 'a']).optional(),
  ages: z.array(z.enum(['10', '20', '30', '40', '50', '60'])).optional()
});
...
export class NaverShoppingInsightClient {
   ...
   */
  public async getCategoryByDevice(request: CategorySingleRequest): Promise<ShoppingInsightResponse> {
    return this.sendRequest<CategorySingleRequest, ShoppingInsightResponse>('/category/device', request);
  }

  /**
   * 쇼핑인사이트 분야 내 성별 트렌드 조회
   */
  public async getCategoryByGender(request: CategorySingleRequest): Promise<ShoppingInsightResponse> {
    return this.sendRequest<CategorySingleRequest, ShoppingInsightResponse>('/category/gender', request);
  }

  /**
   * 쇼핑인사이트 분야 내 연령별 트렌드 조회
   */
  public async getCategoryByAge(request: CategorySingleRequest): Promise<ShoppingInsightResponse> {
    return this.sendRequest<CategorySingleRequest, ShoppingInsightResponse>('/category/age', request);
  }
   ...
   */
  public async getKeywordByDevice(request: KeywordSingleRequest): Promise<ShoppingInsightResponse> {
    return this.sendRequest<KeywordSingleRequest, ShoppingInsightResponse>('/category/keyword/device', request);
  }

  /**
   * 쇼핑인사이트 키워드 성별 트렌드 조회
   */
  public async getKeywordByGender(request: KeywordSingleRequest): Promise<ShoppingInsightResponse> {
    return this.sendRequest<KeywordSingleRequest, ShoppingInsightResponse>('/category/keyword/gender', request);
  }

  /**
   * 쇼핑인사이트 키워드 연령별 트렌드 조회
   */
  public async getKeywordByAge(request: KeywordSingleRequest): Promise<ShoppingInsightResponse> {
    return this.sendRequest<KeywordSingleRequest, ShoppingInsightResponse>('/category/keyword/age', request);
  }
